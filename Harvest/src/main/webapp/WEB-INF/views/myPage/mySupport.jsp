...
<body data-spy="scroll" data-target=".navbar" data-offset="40" id="home">
		...
		<section>
			...
			<ul class="nav nav-tabs">
				...
				<li class="nav-item"><a class="nav-link active"
					aria-current="page"
					href="${pageContext.request.contextPath }/myPage/mySupport">후원한 프로젝트</a></li>
				<li class="nav-item"><a class="nav-link" href="${pageContext.request.contextPath }/myPage/myCreate">내가 만든 프로젝트</a></li>
				<li class="nav-item"><a class="nav-link" href="${pageContext.request.contextPath }/myPage/myFavorite">찜목록 </a></li>
	...
	<div class="container mt-2 mb-4">
		<b style="color: red; white-space: nowrap;">${mySupportCount }</b>개의 프로젝트가 있습니다.
	</div>
...
<!-- 					TODO 결제순, 마감임박순 ListFilter 만들기 -->
					<c:forEach var="mySupportList" items="${mySupportList }">
						<div class="col-md-3 col-sm-6">
						<a href="${pageContext.request.contextPath }/project/projectInfo?idx=${mySupportList.pjIdx }">
							<div class="card text-left">
								<div class="card-header p-0">
									<c:set var="img" value="${mySupportList.img1}" />
									<img src="${pageContext.request.contextPath }/resources/upload/${fn:split(img,'&')[0]}" class="w-100" alt="project_tb.IMG1">
								</div>
								<div class="card-body px-0">
									<p class="my-2">${mySupportList.CATEGORY } | ${mySupportList.creNm }</p>
									<input type="hidden" name="pjIdx" class="pjIdx"
										id="pjIdx_${status.index}" value="${mySupportList.pjIdx}">
									<h5 class="card-title mb-2">${mySupportList.TITLE }</h5>
									<span class="text-danger">${mySupportList.percent }%</span> <small><fmt:formatNumber
											value="${mySupportList.totalAmt}" pattern="#,###" />원</small>
									<div class="progress mt-2 mb-3">
										<div class="progress-bar bg-danger" role="progressbar"
											style="width: ${mySupportList.percent }%" aria-valuenow="25"
											aria-valuemin="0" aria-valuemax="100">
											<c:if test="${mySupportList.expiredDate < 1}">
												<span>오늘 마감</span>
											</c:if>
											<c:if test="${mySupportList.expiredDate > 0}">
												<span>${mySupportList.expiredDate }일 남음</span>
											</c:if>
										</div>
									</div>
								</div>
